...
fun manipulateString(str: String, int: Int): String {
    val validNum = StringBuilder()
    val validStr = StringBuilder()
    var flag: Boolean = false
    str.indices.forEach { i ->
        str[i].digitToIntOrNull()?.let { j ->
            if (j != 0) {
                flag = true
                validNum.append(j.toString())
            }
            if (j == 0 && flag) {
                validNum.append(j.toString())
            }
        } ?: validStr.append(str[i].toString())
    }
    return "${validStr}${int * (validNum.toString().toIntOrNull() ?: 1)}"
}
